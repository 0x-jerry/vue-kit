...
import { createKeyboardEventHandler } from '../keyboard-events'
...
export class KeyboardNavigator {
  #keyHandlers: Array<(ev: KeyboardEvent) => any> = []
  ...
  #initKeyboardEvents() {
    this.#keyHandlers = [
      createKeyboardEventHandler('up', () => this.up()),
      createKeyboardEventHandler('down', () => this.down()),
      createKeyboardEventHandler('left', () => this.left()),
      createKeyboardEventHandler('right', () => this.right()),
      createKeyboardEventHandler('enter', () => {
  ...
  #keyboardHandler = (ev: KeyboardEvent) => {
    this.#keyHandlers.forEach((item) => item(ev))
  }
  ...
  enable() {
    ...
    globalThis.window.addEventListener('keydown', this.#keyboardHandler)
  }

  disable() {
    this.#isEnable = false

    globalThis.window.removeEventListener('keydown', this.#keyboardHandler)
  }
